...
$configureBox = <<-SCRIPT
    echo "deb http://np.archive.ubuntu.com/ubuntu/ xenial main restricted
deb http://np.archive.ubuntu.com/ubuntu/ xenial universe
deb http://np.archive.ubuntu.com/ubuntu/ xenial-updates universe" > /etc/apt/sources.list
    #hostname access for each
    echo "192.168.205.10 k8s-head
    192.168.205.11 k8s-node-1 
    192.168.205.12 k8s-node-2 " >> /etc/hosts
...
EOF
    apt-get update
    apt-get install -y kubelet kubeadm kubectl ntp
...
#    systemctl enable ntp
   ...
   # sudo sed -i '/ swap / s/^\(.*\)$/#\1/g' /etc/fstab
    ...
    echo "Configuring elatic seach virtual memory"
...
Vagrant.configure("2") do |config|
    ...
    servers.each do |opts|
        config.vm.define opts[:name] do |config|
            ...
            if opts[:type] == "master"
                config.vm.network "forwarded_port", guest: 31727, host: 8084
